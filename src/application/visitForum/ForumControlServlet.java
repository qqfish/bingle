...
public class ForumControlServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (func.equals("ini"))
			...
			request.getRequestDispatcher("/jsp/forum/post.jsp").forward(
					request, response);
		} else if (func.equals("newtopic")) {
			newTopic(request.getParameter("tittle"),
			...
			session.setAttribute("lpb", lpb);
			request.getRequestDispatcher("/jsp/forum/reply.jsp").forward(
					request, response);
		} else if (func.equals("newreply")) {
			newReply(request.getParameter("content"),
					Integer.parseInt(request.getParameter("id")),
					request.getParameter("user"), request, response);
		} else if (func.equals("/ViewTopicPageEditReply"))
			editReply(Integer.parseInt(request.getParameter("")),
					Integer.parseInt(request.getParameter("")),
					request.getParameter(""));
		else if (func.equals("/TopicListPageDeleteTopic"))
			deleteTopic(Integer.parseInt(request.getParameter("")),
					request.getParameter(""));
		else if (func.equals("deleteReply"))
	...
	void getTopicList(String topicListName, HttpServletRequest request,
		...
		try {
			...
			for (int i = 0; i < tld.getTopicNum(); i++) {
				table += "<article class='group'><ul class='group'><li class='date'>"
						+ tld.getTopicInfo().get(i).getFirstEditTime()
						+ "</li><li class='posttitle'><header><a href='ForumControlServlet?func=topic&id="
						+ +tld.getTopicInfo().get(i).getTopicId()
						+ "'>"
						+ tld.getTopicInfo().get(i).getTopicName()
						+ "</a></header></li><li class='comments'><a href='ForumControlServlet?func=topic&id="
						+ tld.getTopicInfo().get(i).getTopicId()
						+ "'>"
						+ (tld.getTopicInfo().get(i).getReplyNum() - 1)
						+ "Comments</a></li></ul></article>";
			}
			session.setAttribute("table", table);
			session.setAttribute("tld", tld);
			request.getRequestDispatcher("/jsp/forum/topicList.jsp").forward(
					request, response);
	...
	void getTopic(int topicId, HttpServletRequest request,
			HttpServletResponse response) {
		try {
			IForumSystem fp = new ForumProxy();
			HttpSession session = request.getSession();
			...
			lpb.setUrl(url);
			session.setAttribute("lpb", lpb);
			String table = "<th class='number'>查看:" + td.getViewNum() + "| 回复:"
					+ (td.getReplyNum() - 1) + "</th><th class='title'>"
					+ td.getTopicName() + "</th>";
			for (int i = 0; i < td.getReply().size(); i++) {
				table += "<tr class='text'><td class='left'><p class='name'>"
						+ td.getReply().get(i).getAuthor() + "</p><hr>"
						+ (i + 1) + "楼</td>"
						+ "<td class='right'><p class='time'>发表于: "
						+ td.getReply().get(i).getEditTime();
				if (td.getReply().get(i).getAuthor()
						.equals(session.getAttribute("username"))
						|| (session.getAttribute("admin") != null && session
								.getAttribute("admin").equals("1"))) {
					table += "<a href='/bingle/ForumControlServlet?func=deleteReply&replyId="
							+ td.getReply().get(i).getReplyId()
							+ "&topicId="
							+ td.getTopicId() + "' stype='floar:right;'>删除</a>";
				}
				table += "<hr></p>" + td.getReply().get(i).getContent()
						+ "</td></tr>";
			}
			request.setAttribute("table", table);
			session.setAttribute("td", td);
			request.getRequestDispatcher("/jsp/forum/topic.jsp").forward(
					request, response);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		...
		try {
			...
			for (int i = 0; i < fl.getForumList().size(); i++) {
				...
				table += "<td class='list'><img src='/bingle/img/list.jpg'/></td><td class='textList'>"
						+ "<a href='ForumControlServlet?func=topicList&topic="
						+ fl.getForumList().get(i).getTopicListName()
						+ "'>"
						+ fl.getForumList().get(i).getTopicListName()
						+ "</a><br/><td class='tags'>";
				for (int j = 0; j < fl.getForumList().get(i).getTagName()
						.size()
						&& j < 5; j++) {
					table += "<div class='tag'>"
							+ fl.getForumList().get(i).getTagName().get(j)
							+ "</div>";
				}
				table += "</td>";
				if (i % 2 != 0)
			...
			// fl.getForumList().get(0).getTopicListName());
			// request.setAttribute("tag0",
			// fl.getForumList().get(0).getTagName().get(0));
			request.setAttribute("table", table);
			request.getRequestDispatcher("/jsp/forum/forumList.jsp").forward(
					request, response);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	void newTopic(String topicName, String userName, String content,
			String topicListName, HttpServletRequest request,
			HttpServletResponse response) {
		try {
			IForumSystem fp = new ForumProxy();
			HttpSession session = request.getSession();
			...
			request.getRequestDispatcher("/jsp/forum/workdone.jsp").forward(
					request, response);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
