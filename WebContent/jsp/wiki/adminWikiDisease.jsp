...
<%@ page language="java"%>
<%@ page import="baseUse.wikiData.DiseaseDataList"%>
<!DOCTYPE html>
<html>
	<head>
		...
		<header id="banner" class="body">
			...
			<nav>
				<ul>
					...
					<li class="active"><a href="/bingle/adminDiseaseAndTag?type=disease">管理病症</a></li>
					<li><a href="/bingle/adminDiseaseAndTag?type=tag">管理标签</a></li>
				</ul>
		...
		<section id="container" class="body">
			...
			<table id="search-list-context" >
				...
				<form action="SubmitWikiControlServlet" method="post">
					<tbody>
					<% 
					DiseaseDataList result = (DiseaseDataList) request.getAttribute("result");
					if(result == null){
						%>
						...
						for(int i = 0; i < result.getDisease().size(); i++){ 
					%>
						<tr class="search-result">
							<td><input type="checkbox" name="submitDisease" value="<%=i %>"></td>
							<td><a><%=result.getDisease().get(i).getDiseasename() %></a></td>
							<td><a><%=result.getDisease().get(i).getDiseaseIntro() %></a></td>
							<td>
								<%for(int j = 0; j < result.getDisease().get(i).getTag().size(); j++){ %>
								<div class="tag"><%=result.getDisease().get(i).getTag().get(j).getTagname() %></div></td>
							<%} %>
						</tr>
					<%}} %>	
					</tbody>
